...
franchise <- function(team) {
  ...
  if (missing(team)) {
   return(data.frame(json_franchise$data))
    }
...
hockey_stats <- function(endpoint, team){
  ...
  filtered <- tibble::as_tibble(json_stats$teams)
    ...
    filtered <- data.frame(json_stats$teams)
  }
    ...
    filtered <- data.frame(json_stats$teams$teamStats)
  }
...
skater_position2 +  stat_summary(fun.y = mean, geom = "line", lwd= 1.5, aes(group = data.activePlayer, col= data.activePlayer)) + labs(c="Position", y= "Games Played", title = "Mean games played by position for the Anaheim Ducks by active player")
#scatterplot
